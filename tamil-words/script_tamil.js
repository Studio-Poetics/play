/*******************************************************
 * Word Sprint (Anagram Challenge) - Tamil Version
 *
...
async function loadWords() {
  try {
    ...
    const response = await fetch("tamil_words.json");
    if (!response.ok) {
...
function startGame() {
  ...
  if (
    ...
    alert("சொல் பட்டியல் இன்னும் ஏற்றப்படவில்லை!");
    return;
  }
  ...
  timerDisplay.textContent = `நேரம்: ${timeRemaining}`;
...
function failRound(reason) {
  ...
  let answerText = `ஒரு சரியான சொல்: ${currentWord}`;
  ...
  if (allAnagrams.length > 0) {
    ...
    answerText += ` | மற்றவை: ${limitedAnagrams.join(", ")}`;
    if (allAnagrams.length > 4) {
      answerText += ` (+${allAnagrams.length - 4} மேலும்)`;
    }
  }
...
function checkAnswer() {
  ...
  if (isValidAnagram(userGuess, currentWord) && dictionarySet.has(userGuess)) {
    ...
    let successText = `சரி! சொல்: ${currentWord}`;

    if (otherAnagrams.length > 0) {
      const limitedAnagrams = otherAnagrams.slice(0, 3);
      successText += ` | மற்ற சொற்கள்: ${limitedAnagrams.join(", ")}`;
      if (otherAnagrams.length > 3) {
        successText += ` (+${otherAnagrams.length - 3} மேலும்)`;
      }
    }
...
function updateScoreDisplay() {
  scoreDisplay.textContent  = `மதிப்பெண்: ${score}`;
  streakDisplay.textContent = `தொடர்: ${streak}`;
}
...
function endGame() {
  ...
  clearInterval(timer);
  timer = null;
  scrambledLetters.textContent = "விளையாட்டு முடிந்தது!";
